...
module.exports = MangerList
function MangerList(managerSize) {
   ...
   */
  this.managerData = function () {
    return {data:"some manager data"};
  }
  /*
   ...
   */
  this.plebData = function () {
    return {data:"some pleb data"};
  }
  /*
  ...
  this.introduce = function (socket,userId) {
    ...
    for(var managerId in storage) { //find manager with space, add pleb
      if(storage[managerId].plebs.length < this.managerSize) {
        ...
        socket.emit("Pleb_Setup",this.plebData());
