...
module.exports = function ManagerList(managerSize) {
  ...
  this.plebRecieved = function (managerId, plebSocketId) {
    var pendingIdx = where(this.storage[managerId].pending, function (pendingObj, idx, col) {
      if (pendingObj.socket.id === plebSocketId) {
        return true;
      }
      return false;
    });
    if (pendingIdx !== -1) {
      ...
      this.storage[managerId].pending[pendingIdx].socket.disconnect();
      return this.storage[managerId].pending.splice(pendingIdx, 1);
    }
    return false;
  }
