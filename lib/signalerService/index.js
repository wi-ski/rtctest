...
function Server(srv, xx,opts) {
  if (!(this instanceof Server)) return new Server(srv, xx,opts);
  ...
  opts = opts || {};
  this.nsps = {};
  this.path(opts.path || '/signalerService/speakEasySignaler.js');
  this.serveClient(false !== opts.serveClient);
  this.origins(opts.origins || '*:*');
  if (srv) {
   this.attach(srv, xx,opts); 
  }
}
...
Server.prototype.listen =
  Server.prototype.attach = function (srv, xx ,opts) {

    if ('function' == typeof srv) {
      throw new Error('You are trying to attach reliable-signaler to an express request handler function. Please pass a http.Server instance.');
    }
    ...
    require('./socketHandler.js').easySignaler(srv,xx,opts.socketCallback || function () {});
