...
function easySignaler(app,express) {
  ...
  express.get('/confirm/:manid/:plebid',function(req,res){
    console.log(req)
    ManagerList.plebRecieved(req.params.manid,req.params.plebid)
    res.send(JSON.stringify({success:true,p:req.params.plebid,m:req.params.manid}))
  });
  ...
  io.on('connection', function (socket) {
     ...
     */
    socket.on('pc', function (plebSocketId) { //having two of these might be overkill
      ManagerList.plebRecieved(socket.id, plebSocketId)
    });
